                ...
                */
                ...
                _.each(_section.data.fields, function(_field){
                  ...
                  else if(_field.type === 'multiCheckbox')
                  {
                    ...
                    var multiArr = [];
                    var multi_uuid = [];
                    var val2 = _.filter(obs_data,function(obs){
                      if(obs.concept.uuid === convertKey_to_uuid(field_key.split('_')[1]))
                        return obs;
                    });
                    if (val2 !== undefined) {
                      _.each(val2, function(obs){
...
<<<<<<< HEAD
                        ...
                        if(group_data !== undefined)
                        {
                          if(_group_field.type !== 'multiCheckbox')
                          {
                             if(_.contains(_group_field.key, 'obsDate'))
                             {
                               var val4 = _.find(group_data[0].groupMembers, function(obs){
                                 if(obs.concept.uuid === convertKey_to_uuid(_group_field.key.split('_')[1]))
                                  return obs;
                               });
                               if(val4 !== undefined)
                               {
                                 group_val[_group_field.key] = new Date(val4.obsDatetime);
                                 _group_field.data['init_val'] = val4.obsDatetime;
                                 //obs uuid
                                 _group_field.data['uuid'] = val4.uuid;
                               }
                             }
                             else {
                              var val5;
